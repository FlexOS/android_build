...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^flex_") ; then
       FLEX_BUILD=$(echo -n $1 | sed -e 's/^flex_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $FLEX_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       FLEX_BUILD=
    fi
    export FLEX_BUILD
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var FLEX_VERSION)
        ZIPFILE=FlexOS-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.flex.device=$FLEX_BUILD");
    then
...
function installboot()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$FLEX_BUILD");
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $FLEX_FIXUP_COMMON_OUT ]; then
